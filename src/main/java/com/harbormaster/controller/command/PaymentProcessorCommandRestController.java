/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * FinTech Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>PaymentProcessorCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity PaymentProcessor.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>FinTech Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core digital banking, payments, wallets, lending, investments, FX, compliance, and risk management capabilities.</td></tr>
 *          <tr><td>industry</td><td>FinTech</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	PaymentProcessorService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreatePaymentProcessorCommand</li>
 *      <li>UpdatePaymentProcessorCommand</li>
 *      <liDeletePaymentProcessorCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/PaymentProcessorCommand")
public class PaymentProcessorCommandRestController extends BaseSpringRestController {

	public PaymentProcessorCommandRestController( PaymentProcessorService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a PaymentProcessor.
     * @param		PaymentProcessor	paymentProcessor
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreatePaymentProcessorCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createPaymentProcessor( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a PaymentProcessor.  if no key provided, calls create, otherwise calls save
     * @param		PaymentProcessor paymentProcessor
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdatePaymentProcessorCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdatePaymentProcessorCommand
			// -----------------------------------------------
			completableFuture = service.updatePaymentProcessor(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "PaymentProcessorController:update() - successfully update PaymentProcessor - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a PaymentProcessor entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID paymentProcessorId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeletePaymentProcessorCommand command = new DeletePaymentProcessorCommand( paymentProcessorId );

    	try {
        	PaymentProcessorService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted PaymentProcessor with key " + command.getPaymentProcessorId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	


    /**
     * save Institutions on PaymentProcessor
     * @param		command AssignInstitutionsToPaymentProcessorCommand
     */     
	@PutMapping("/addToInstitutions")
	public void addToInstitutions( @RequestBody(required=true) AssignInstitutionsToPaymentProcessorCommand command ) {
		try {
			service.addToInstitutions( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Institutions", exc );
		}
	}

    /**
     * remove Institutions on PaymentProcessor
     * @param		command RemoveInstitutionsFromPaymentProcessorCommand
     */     	
	@PutMapping("/removeFromInstitutions")
	public void removeFromInstitutions( 	@RequestBody(required=true) RemoveInstitutionsFromPaymentProcessorCommand command )
	{		
		try {
			service.removeFromInstitutions( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Institutions", exc );
		}
	}

    /**
     * save Contracts on PaymentProcessor
     * @param		command AssignContractsToPaymentProcessorCommand
     */     
	@PutMapping("/addToContracts")
	public void addToContracts( @RequestBody(required=true) AssignContractsToPaymentProcessorCommand command ) {
		try {
			service.addToContracts( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Contracts", exc );
		}
	}

    /**
     * remove Contracts on PaymentProcessor
     * @param		command RemoveContractsFromPaymentProcessorCommand
     */     	
	@PutMapping("/removeFromContracts")
	public void removeFromContracts( 	@RequestBody(required=true) RemoveContractsFromPaymentProcessorCommand command )
	{		
		try {
			service.removeFromContracts( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Contracts", exc );
		}
	}

    /**
     * save Settlements on PaymentProcessor
     * @param		command AssignSettlementsToPaymentProcessorCommand
     */     
	@PutMapping("/addToSettlements")
	public void addToSettlements( @RequestBody(required=true) AssignSettlementsToPaymentProcessorCommand command ) {
		try {
			service.addToSettlements( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Settlements", exc );
		}
	}

    /**
     * remove Settlements on PaymentProcessor
     * @param		command RemoveSettlementsFromPaymentProcessorCommand
     */     	
	@PutMapping("/removeFromSettlements")
	public void removeFromSettlements( 	@RequestBody(required=true) RemoveSettlementsFromPaymentProcessorCommand command )
	{		
		try {
			service.removeFromSettlements( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Settlements", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected PaymentProcessor paymentProcessor = null;
	protected PaymentProcessorService service = null;
    private static final Logger LOGGER = Logger.getLogger(PaymentProcessorCommandRestController.class.getName());
    
}
