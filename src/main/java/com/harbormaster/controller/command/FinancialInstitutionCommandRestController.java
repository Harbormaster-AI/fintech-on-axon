/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * FinTech Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>FinancialInstitutionCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity FinancialInstitution.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>FinTech Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core digital banking, payments, wallets, lending, investments, FX, compliance, and risk management capabilities.</td></tr>
 *          <tr><td>industry</td><td>FinTech</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	FinancialInstitutionService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateFinancialInstitutionCommand</li>
 *      <li>UpdateFinancialInstitutionCommand</li>
 *      <liDeleteFinancialInstitutionCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/FinancialInstitutionCommand")
public class FinancialInstitutionCommandRestController extends BaseSpringRestController {

	public FinancialInstitutionCommandRestController( FinancialInstitutionService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a FinancialInstitution.
     * @param		FinancialInstitution	financialInstitution
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateFinancialInstitutionCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createFinancialInstitution( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a FinancialInstitution.  if no key provided, calls create, otherwise calls save
     * @param		FinancialInstitution financialInstitution
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateFinancialInstitutionCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateFinancialInstitutionCommand
			// -----------------------------------------------
			completableFuture = service.updateFinancialInstitution(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "FinancialInstitutionController:update() - successfully update FinancialInstitution - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a FinancialInstitution entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID financialInstitutionId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteFinancialInstitutionCommand command = new DeleteFinancialInstitutionCommand( financialInstitutionId );

    	try {
        	FinancialInstitutionService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted FinancialInstitution with key " + command.getFinancialInstitutionId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	


    /**
     * save Branches on FinancialInstitution
     * @param		command AssignBranchesToFinancialInstitutionCommand
     */     
	@PutMapping("/addToBranches")
	public void addToBranches( @RequestBody(required=true) AssignBranchesToFinancialInstitutionCommand command ) {
		try {
			service.addToBranches( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Branches", exc );
		}
	}

    /**
     * remove Branches on FinancialInstitution
     * @param		command RemoveBranchesFromFinancialInstitutionCommand
     */     	
	@PutMapping("/removeFromBranches")
	public void removeFromBranches( 	@RequestBody(required=true) RemoveBranchesFromFinancialInstitutionCommand command )
	{		
		try {
			service.removeFromBranches( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Branches", exc );
		}
	}

    /**
     * save Customers on FinancialInstitution
     * @param		command AssignCustomersToFinancialInstitutionCommand
     */     
	@PutMapping("/addToCustomers")
	public void addToCustomers( @RequestBody(required=true) AssignCustomersToFinancialInstitutionCommand command ) {
		try {
			service.addToCustomers( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Customers", exc );
		}
	}

    /**
     * remove Customers on FinancialInstitution
     * @param		command RemoveCustomersFromFinancialInstitutionCommand
     */     	
	@PutMapping("/removeFromCustomers")
	public void removeFromCustomers( 	@RequestBody(required=true) RemoveCustomersFromFinancialInstitutionCommand command )
	{		
		try {
			service.removeFromCustomers( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Customers", exc );
		}
	}

    /**
     * save ProductOfferings on FinancialInstitution
     * @param		command AssignProductOfferingsToFinancialInstitutionCommand
     */     
	@PutMapping("/addToProductOfferings")
	public void addToProductOfferings( @RequestBody(required=true) AssignProductOfferingsToFinancialInstitutionCommand command ) {
		try {
			service.addToProductOfferings( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set ProductOfferings", exc );
		}
	}

    /**
     * remove ProductOfferings on FinancialInstitution
     * @param		command RemoveProductOfferingsFromFinancialInstitutionCommand
     */     	
	@PutMapping("/removeFromProductOfferings")
	public void removeFromProductOfferings( 	@RequestBody(required=true) RemoveProductOfferingsFromFinancialInstitutionCommand command )
	{		
		try {
			service.removeFromProductOfferings( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set ProductOfferings", exc );
		}
	}

    /**
     * save PaymentProcessors on FinancialInstitution
     * @param		command AssignPaymentProcessorsToFinancialInstitutionCommand
     */     
	@PutMapping("/addToPaymentProcessors")
	public void addToPaymentProcessors( @RequestBody(required=true) AssignPaymentProcessorsToFinancialInstitutionCommand command ) {
		try {
			service.addToPaymentProcessors( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set PaymentProcessors", exc );
		}
	}

    /**
     * remove PaymentProcessors on FinancialInstitution
     * @param		command RemovePaymentProcessorsFromFinancialInstitutionCommand
     */     	
	@PutMapping("/removeFromPaymentProcessors")
	public void removeFromPaymentProcessors( 	@RequestBody(required=true) RemovePaymentProcessorsFromFinancialInstitutionCommand command )
	{		
		try {
			service.removeFromPaymentProcessors( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set PaymentProcessors", exc );
		}
	}

    /**
     * save CompliancePolicies on FinancialInstitution
     * @param		command AssignCompliancePoliciesToFinancialInstitutionCommand
     */     
	@PutMapping("/addToCompliancePolicies")
	public void addToCompliancePolicies( @RequestBody(required=true) AssignCompliancePoliciesToFinancialInstitutionCommand command ) {
		try {
			service.addToCompliancePolicies( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set CompliancePolicies", exc );
		}
	}

    /**
     * remove CompliancePolicies on FinancialInstitution
     * @param		command RemoveCompliancePoliciesFromFinancialInstitutionCommand
     */     	
	@PutMapping("/removeFromCompliancePolicies")
	public void removeFromCompliancePolicies( 	@RequestBody(required=true) RemoveCompliancePoliciesFromFinancialInstitutionCommand command )
	{		
		try {
			service.removeFromCompliancePolicies( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set CompliancePolicies", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected FinancialInstitution financialInstitution = null;
	protected FinancialInstitutionService service = null;
    private static final Logger LOGGER = Logger.getLogger(FinancialInstitutionCommandRestController.class.getName());
    
}
