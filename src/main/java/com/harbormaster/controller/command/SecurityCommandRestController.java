/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * FinTech Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>SecurityCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity Security.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>FinTech Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core digital banking, payments, wallets, lending, investments, FX, compliance, and risk management capabilities.</td></tr>
 *          <tr><td>industry</td><td>FinTech</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	SecurityService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateSecurityCommand</li>
 *      <li>UpdateSecurityCommand</li>
 *      <liDeleteSecurityCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/SecurityCommand")
public class SecurityCommandRestController extends BaseSpringRestController {

	public SecurityCommandRestController( SecurityService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a Security.
     * @param		Security	security
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateSecurityCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createSecurity( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a Security.  if no key provided, calls create, otherwise calls save
     * @param		Security security
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateSecurityCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateSecurityCommand
			// -----------------------------------------------
			completableFuture = service.updateSecurity(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "SecurityController:update() - successfully update Security - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a Security entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID securityId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteSecurityCommand command = new DeleteSecurityCommand( securityId );

    	try {
        	SecurityService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted Security with key " + command.getSecurityId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	


    /**
     * save Positions on Security
     * @param		command AssignPositionsToSecurityCommand
     */     
	@PutMapping("/addToPositions")
	public void addToPositions( @RequestBody(required=true) AssignPositionsToSecurityCommand command ) {
		try {
			service.addToPositions( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Positions", exc );
		}
	}

    /**
     * remove Positions on Security
     * @param		command RemovePositionsFromSecurityCommand
     */     	
	@PutMapping("/removeFromPositions")
	public void removeFromPositions( 	@RequestBody(required=true) RemovePositionsFromSecurityCommand command )
	{		
		try {
			service.removeFromPositions( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Positions", exc );
		}
	}

    /**
     * save Trades on Security
     * @param		command AssignTradesToSecurityCommand
     */     
	@PutMapping("/addToTrades")
	public void addToTrades( @RequestBody(required=true) AssignTradesToSecurityCommand command ) {
		try {
			service.addToTrades( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Trades", exc );
		}
	}

    /**
     * remove Trades on Security
     * @param		command RemoveTradesFromSecurityCommand
     */     	
	@PutMapping("/removeFromTrades")
	public void removeFromTrades( 	@RequestBody(required=true) RemoveTradesFromSecurityCommand command )
	{		
		try {
			service.removeFromTrades( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Trades", exc );
		}
	}

    /**
     * save Orders on Security
     * @param		command AssignOrdersToSecurityCommand
     */     
	@PutMapping("/addToOrders")
	public void addToOrders( @RequestBody(required=true) AssignOrdersToSecurityCommand command ) {
		try {
			service.addToOrders( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Orders", exc );
		}
	}

    /**
     * remove Orders on Security
     * @param		command RemoveOrdersFromSecurityCommand
     */     	
	@PutMapping("/removeFromOrders")
	public void removeFromOrders( 	@RequestBody(required=true) RemoveOrdersFromSecurityCommand command )
	{		
		try {
			service.removeFromOrders( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Orders", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected Security security = null;
	protected SecurityService service = null;
    private static final Logger LOGGER = Logger.getLogger(SecurityCommandRestController.class.getName());
    
}
