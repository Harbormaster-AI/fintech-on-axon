/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * FinTech Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>InvestmentAccountCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity InvestmentAccount.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>FinTech Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core digital banking, payments, wallets, lending, investments, FX, compliance, and risk management capabilities.</td></tr>
 *          <tr><td>industry</td><td>FinTech</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	InvestmentAccountService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateInvestmentAccountCommand</li>
 *      <li>UpdateInvestmentAccountCommand</li>
 *      <liDeleteInvestmentAccountCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/InvestmentAccountCommand")
public class InvestmentAccountCommandRestController extends BaseSpringRestController {

	public InvestmentAccountCommandRestController( InvestmentAccountService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a InvestmentAccount.
     * @param		InvestmentAccount	investmentAccount
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateInvestmentAccountCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createInvestmentAccount( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a InvestmentAccount.  if no key provided, calls create, otherwise calls save
     * @param		InvestmentAccount investmentAccount
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateInvestmentAccountCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateInvestmentAccountCommand
			// -----------------------------------------------
			completableFuture = service.updateInvestmentAccount(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "InvestmentAccountController:update() - successfully update InvestmentAccount - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a InvestmentAccount entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID investmentAccountId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteInvestmentAccountCommand command = new DeleteInvestmentAccountCommand( investmentAccountId );

    	try {
        	InvestmentAccountService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted InvestmentAccount with key " + command.getInvestmentAccountId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Portfolio on InvestmentAccount
     * @param		command AssignPortfolioToInvestmentAccountCommand
     */     
	@PutMapping("/assignPortfolio")
	public void assignPortfolio( @RequestBody AssignPortfolioToInvestmentAccountCommand command ) {
		try {
			service.assignPortfolio( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Portfolio", exc );
        }
	}

    /**
     * unassign Portfolio on InvestmentAccount
     * @param		 command UnAssignPortfolioFromInvestmentAccountCommand
     */     
	@PutMapping("/unAssignPortfolio")
	public void unAssignPortfolio( @RequestBody(required=true)  UnAssignPortfolioFromInvestmentAccountCommand command ) {
		try {
			service.unAssignPortfolio( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Portfolio", exc );
		}
	}
	

    /**
     * save Trades on InvestmentAccount
     * @param		command AssignTradesToInvestmentAccountCommand
     */     
	@PutMapping("/addToTrades")
	public void addToTrades( @RequestBody(required=true) AssignTradesToInvestmentAccountCommand command ) {
		try {
			service.addToTrades( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Trades", exc );
		}
	}

    /**
     * remove Trades on InvestmentAccount
     * @param		command RemoveTradesFromInvestmentAccountCommand
     */     	
	@PutMapping("/removeFromTrades")
	public void removeFromTrades( 	@RequestBody(required=true) RemoveTradesFromInvestmentAccountCommand command )
	{		
		try {
			service.removeFromTrades( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Trades", exc );
		}
	}

    /**
     * save Orders on InvestmentAccount
     * @param		command AssignOrdersToInvestmentAccountCommand
     */     
	@PutMapping("/addToOrders")
	public void addToOrders( @RequestBody(required=true) AssignOrdersToInvestmentAccountCommand command ) {
		try {
			service.addToOrders( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Orders", exc );
		}
	}

    /**
     * remove Orders on InvestmentAccount
     * @param		command RemoveOrdersFromInvestmentAccountCommand
     */     	
	@PutMapping("/removeFromOrders")
	public void removeFromOrders( 	@RequestBody(required=true) RemoveOrdersFromInvestmentAccountCommand command )
	{		
		try {
			service.removeFromOrders( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Orders", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected InvestmentAccount investmentAccount = null;
	protected InvestmentAccountService service = null;
    private static final Logger LOGGER = Logger.getLogger(InvestmentAccountCommandRestController.class.getName());
    
}
