/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * FinTech Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>SettlementBatchCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity SettlementBatch.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>FinTech Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core digital banking, payments, wallets, lending, investments, FX, compliance, and risk management capabilities.</td></tr>
 *          <tr><td>industry</td><td>FinTech</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	SettlementBatchService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateSettlementBatchCommand</li>
 *      <li>UpdateSettlementBatchCommand</li>
 *      <liDeleteSettlementBatchCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/SettlementBatchCommand")
public class SettlementBatchCommandRestController extends BaseSpringRestController {

	public SettlementBatchCommandRestController( SettlementBatchService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a SettlementBatch.
     * @param		SettlementBatch	settlementBatch
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateSettlementBatchCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createSettlementBatch( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a SettlementBatch.  if no key provided, calls create, otherwise calls save
     * @param		SettlementBatch settlementBatch
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateSettlementBatchCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateSettlementBatchCommand
			// -----------------------------------------------
			completableFuture = service.updateSettlementBatch(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "SettlementBatchController:update() - successfully update SettlementBatch - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a SettlementBatch entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID settlementBatchId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteSettlementBatchCommand command = new DeleteSettlementBatchCommand( settlementBatchId );

    	try {
        	SettlementBatchService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted SettlementBatch with key " + command.getSettlementBatchId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Processor on SettlementBatch
     * @param		command AssignProcessorToSettlementBatchCommand
     */     
	@PutMapping("/assignProcessor")
	public void assignProcessor( @RequestBody AssignProcessorToSettlementBatchCommand command ) {
		try {
			service.assignProcessor( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Processor", exc );
        }
	}

    /**
     * unassign Processor on SettlementBatch
     * @param		 command UnAssignProcessorFromSettlementBatchCommand
     */     
	@PutMapping("/unAssignProcessor")
	public void unAssignProcessor( @RequestBody(required=true)  UnAssignProcessorFromSettlementBatchCommand command ) {
		try {
			service.unAssignProcessor( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Processor", exc );
		}
	}
	
    /**
     * save Merchant on SettlementBatch
     * @param		command AssignMerchantToSettlementBatchCommand
     */     
	@PutMapping("/assignMerchant")
	public void assignMerchant( @RequestBody AssignMerchantToSettlementBatchCommand command ) {
		try {
			service.assignMerchant( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Merchant", exc );
        }
	}

    /**
     * unassign Merchant on SettlementBatch
     * @param		 command UnAssignMerchantFromSettlementBatchCommand
     */     
	@PutMapping("/unAssignMerchant")
	public void unAssignMerchant( @RequestBody(required=true)  UnAssignMerchantFromSettlementBatchCommand command ) {
		try {
			service.unAssignMerchant( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Merchant", exc );
		}
	}
	

    /**
     * save Payouts on SettlementBatch
     * @param		command AssignPayoutsToSettlementBatchCommand
     */     
	@PutMapping("/addToPayouts")
	public void addToPayouts( @RequestBody(required=true) AssignPayoutsToSettlementBatchCommand command ) {
		try {
			service.addToPayouts( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Payouts", exc );
		}
	}

    /**
     * remove Payouts on SettlementBatch
     * @param		command RemovePayoutsFromSettlementBatchCommand
     */     	
	@PutMapping("/removeFromPayouts")
	public void removeFromPayouts( 	@RequestBody(required=true) RemovePayoutsFromSettlementBatchCommand command )
	{		
		try {
			service.removeFromPayouts( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Payouts", exc );
		}
	}

    /**
     * save Transactions on SettlementBatch
     * @param		command AssignTransactionsToSettlementBatchCommand
     */     
	@PutMapping("/addToTransactions")
	public void addToTransactions( @RequestBody(required=true) AssignTransactionsToSettlementBatchCommand command ) {
		try {
			service.addToTransactions( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Transactions", exc );
		}
	}

    /**
     * remove Transactions on SettlementBatch
     * @param		command RemoveTransactionsFromSettlementBatchCommand
     */     	
	@PutMapping("/removeFromTransactions")
	public void removeFromTransactions( 	@RequestBody(required=true) RemoveTransactionsFromSettlementBatchCommand command )
	{		
		try {
			service.removeFromTransactions( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Transactions", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected SettlementBatch settlementBatch = null;
	protected SettlementBatchService service = null;
    private static final Logger LOGGER = Logger.getLogger(SettlementBatchCommandRestController.class.getName());
    
}
