/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * FinTech Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>TradeOrderCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity TradeOrder.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>FinTech Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core digital banking, payments, wallets, lending, investments, FX, compliance, and risk management capabilities.</td></tr>
 *          <tr><td>industry</td><td>FinTech</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	TradeOrderService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateTradeOrderCommand</li>
 *      <li>UpdateTradeOrderCommand</li>
 *      <liDeleteTradeOrderCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/TradeOrderCommand")
public class TradeOrderCommandRestController extends BaseSpringRestController {

	public TradeOrderCommandRestController( TradeOrderService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a TradeOrder.
     * @param		TradeOrder	tradeOrder
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateTradeOrderCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createTradeOrder( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a TradeOrder.  if no key provided, calls create, otherwise calls save
     * @param		TradeOrder tradeOrder
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateTradeOrderCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateTradeOrderCommand
			// -----------------------------------------------
			completableFuture = service.updateTradeOrder(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "TradeOrderController:update() - successfully update TradeOrder - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a TradeOrder entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID tradeOrderId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteTradeOrderCommand command = new DeleteTradeOrderCommand( tradeOrderId );

    	try {
        	TradeOrderService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted TradeOrder with key " + command.getTradeOrderId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Portfolio on TradeOrder
     * @param		command AssignPortfolioToTradeOrderCommand
     */     
	@PutMapping("/assignPortfolio")
	public void assignPortfolio( @RequestBody AssignPortfolioToTradeOrderCommand command ) {
		try {
			service.assignPortfolio( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Portfolio", exc );
        }
	}

    /**
     * unassign Portfolio on TradeOrder
     * @param		 command UnAssignPortfolioFromTradeOrderCommand
     */     
	@PutMapping("/unAssignPortfolio")
	public void unAssignPortfolio( @RequestBody(required=true)  UnAssignPortfolioFromTradeOrderCommand command ) {
		try {
			service.unAssignPortfolio( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Portfolio", exc );
		}
	}
	
    /**
     * save Security on TradeOrder
     * @param		command AssignSecurityToTradeOrderCommand
     */     
	@PutMapping("/assignSecurity")
	public void assignSecurity( @RequestBody AssignSecurityToTradeOrderCommand command ) {
		try {
			service.assignSecurity( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Security", exc );
        }
	}

    /**
     * unassign Security on TradeOrder
     * @param		 command UnAssignSecurityFromTradeOrderCommand
     */     
	@PutMapping("/unAssignSecurity")
	public void unAssignSecurity( @RequestBody(required=true)  UnAssignSecurityFromTradeOrderCommand command ) {
		try {
			service.unAssignSecurity( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Security", exc );
		}
	}
	

    /**
     * save Trades on TradeOrder
     * @param		command AssignTradesToTradeOrderCommand
     */     
	@PutMapping("/addToTrades")
	public void addToTrades( @RequestBody(required=true) AssignTradesToTradeOrderCommand command ) {
		try {
			service.addToTrades( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Trades", exc );
		}
	}

    /**
     * remove Trades on TradeOrder
     * @param		command RemoveTradesFromTradeOrderCommand
     */     	
	@PutMapping("/removeFromTrades")
	public void removeFromTrades( 	@RequestBody(required=true) RemoveTradesFromTradeOrderCommand command )
	{		
		try {
			service.removeFromTrades( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Trades", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected TradeOrder tradeOrder = null;
	protected TradeOrderService service = null;
    private static final Logger LOGGER = Logger.getLogger(TradeOrderCommandRestController.class.getName());
    
}
