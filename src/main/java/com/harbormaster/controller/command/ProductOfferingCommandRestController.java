/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * FinTech Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>ProductOfferingCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity ProductOffering.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>FinTech Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core digital banking, payments, wallets, lending, investments, FX, compliance, and risk management capabilities.</td></tr>
 *          <tr><td>industry</td><td>FinTech</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	ProductOfferingService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateProductOfferingCommand</li>
 *      <li>UpdateProductOfferingCommand</li>
 *      <liDeleteProductOfferingCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/ProductOfferingCommand")
public class ProductOfferingCommandRestController extends BaseSpringRestController {

	public ProductOfferingCommandRestController( ProductOfferingService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a ProductOffering.
     * @param		ProductOffering	productOffering
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateProductOfferingCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createProductOffering( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a ProductOffering.  if no key provided, calls create, otherwise calls save
     * @param		ProductOffering productOffering
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateProductOfferingCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateProductOfferingCommand
			// -----------------------------------------------
			completableFuture = service.updateProductOffering(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "ProductOfferingController:update() - successfully update ProductOffering - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a ProductOffering entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID productOfferingId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteProductOfferingCommand command = new DeleteProductOfferingCommand( productOfferingId );

    	try {
        	ProductOfferingService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted ProductOffering with key " + command.getProductOfferingId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Institution on ProductOffering
     * @param		command AssignInstitutionToProductOfferingCommand
     */     
	@PutMapping("/assignInstitution")
	public void assignInstitution( @RequestBody AssignInstitutionToProductOfferingCommand command ) {
		try {
			service.assignInstitution( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Institution", exc );
        }
	}

    /**
     * unassign Institution on ProductOffering
     * @param		 command UnAssignInstitutionFromProductOfferingCommand
     */     
	@PutMapping("/unAssignInstitution")
	public void unAssignInstitution( @RequestBody(required=true)  UnAssignInstitutionFromProductOfferingCommand command ) {
		try {
			service.unAssignInstitution( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Institution", exc );
		}
	}
	

    /**
     * save PricingPlans on ProductOffering
     * @param		command AssignPricingPlansToProductOfferingCommand
     */     
	@PutMapping("/addToPricingPlans")
	public void addToPricingPlans( @RequestBody(required=true) AssignPricingPlansToProductOfferingCommand command ) {
		try {
			service.addToPricingPlans( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set PricingPlans", exc );
		}
	}

    /**
     * remove PricingPlans on ProductOffering
     * @param		command RemovePricingPlansFromProductOfferingCommand
     */     	
	@PutMapping("/removeFromPricingPlans")
	public void removeFromPricingPlans( 	@RequestBody(required=true) RemovePricingPlansFromProductOfferingCommand command )
	{		
		try {
			service.removeFromPricingPlans( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set PricingPlans", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected ProductOffering productOffering = null;
	protected ProductOfferingService service = null;
    private static final Logger LOGGER = Logger.getLogger(ProductOfferingCommandRestController.class.getName());
    
}
