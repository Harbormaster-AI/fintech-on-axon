/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * FinTech Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>PaymentContractCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity PaymentContract.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>FinTech Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core digital banking, payments, wallets, lending, investments, FX, compliance, and risk management capabilities.</td></tr>
 *          <tr><td>industry</td><td>FinTech</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	PaymentContractService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreatePaymentContractCommand</li>
 *      <li>UpdatePaymentContractCommand</li>
 *      <liDeletePaymentContractCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/PaymentContractCommand")
public class PaymentContractCommandRestController extends BaseSpringRestController {

	public PaymentContractCommandRestController( PaymentContractService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a PaymentContract.
     * @param		PaymentContract	paymentContract
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreatePaymentContractCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createPaymentContract( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a PaymentContract.  if no key provided, calls create, otherwise calls save
     * @param		PaymentContract paymentContract
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdatePaymentContractCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdatePaymentContractCommand
			// -----------------------------------------------
			completableFuture = service.updatePaymentContract(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "PaymentContractController:update() - successfully update PaymentContract - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a PaymentContract entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID paymentContractId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeletePaymentContractCommand command = new DeletePaymentContractCommand( paymentContractId );

    	try {
        	PaymentContractService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted PaymentContract with key " + command.getPaymentContractId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Merchant on PaymentContract
     * @param		command AssignMerchantToPaymentContractCommand
     */     
	@PutMapping("/assignMerchant")
	public void assignMerchant( @RequestBody AssignMerchantToPaymentContractCommand command ) {
		try {
			service.assignMerchant( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Merchant", exc );
        }
	}

    /**
     * unassign Merchant on PaymentContract
     * @param		 command UnAssignMerchantFromPaymentContractCommand
     */     
	@PutMapping("/unAssignMerchant")
	public void unAssignMerchant( @RequestBody(required=true)  UnAssignMerchantFromPaymentContractCommand command ) {
		try {
			service.unAssignMerchant( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Merchant", exc );
		}
	}
	
    /**
     * save Acquirer on PaymentContract
     * @param		command AssignAcquirerToPaymentContractCommand
     */     
	@PutMapping("/assignAcquirer")
	public void assignAcquirer( @RequestBody AssignAcquirerToPaymentContractCommand command ) {
		try {
			service.assignAcquirer( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Acquirer", exc );
        }
	}

    /**
     * unassign Acquirer on PaymentContract
     * @param		 command UnAssignAcquirerFromPaymentContractCommand
     */     
	@PutMapping("/unAssignAcquirer")
	public void unAssignAcquirer( @RequestBody(required=true)  UnAssignAcquirerFromPaymentContractCommand command ) {
		try {
			service.unAssignAcquirer( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Acquirer", exc );
		}
	}
	


//************************************************************************    
// Attributes
//************************************************************************
    protected PaymentContract paymentContract = null;
	protected PaymentContractService service = null;
    private static final Logger LOGGER = Logger.getLogger(PaymentContractCommandRestController.class.getName());
    
}
