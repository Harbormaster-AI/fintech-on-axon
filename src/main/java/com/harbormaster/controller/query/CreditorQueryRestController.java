/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * FinTech Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.query;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;
/**
 * <h2>CreditorQueryRestController as REST API</h2>
 * <p>
 * Implements Spring Controller query CQRS processing for entity Creditor.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>FinTech Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core digital banking, payments, wallets, lending, investments, FX, compliance, and risk management capabilities.</td></tr>
 *          <tr><td>industry</td><td>FinTech</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	CreditorService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateCreditorCommand</li>
 *      <li></li>UpdateCreditorCommand</li>
 *      <liDeleteCreditorCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */
@CrossOrigin
@RestController
@RequestMapping("/CreditorQuery")
public class CreditorQueryRestController extends BaseSpringRestController {
	
    /**
     * Handles loading a Creditor using a UUID
     * @param		UUID creditorId
     * @return		Creditor
     */    
    @GetMapping("/load")
    public Creditor load( @RequestParam(required=true) UUID creditorId ) {
    	Creditor entity = null;

    	try {  
    		entity = CreditorService.getCreditorInstance().getCreditor( new CreditorFetchOneSummary( creditorId ) );
        }
        catch( Throwable exc ) {
            LOGGER.log( Level.WARNING, "failed to load Creditor using Id " + creditorId );
            return null;
        }

        return entity;
    }

    /**
     * Handles loading all Creditor business objects
     * @return		Set<Creditor>
     */
    @GetMapping("/")
    public List<Creditor> loadAll() {                
    	List<Creditor> creditorList = null;
        
    	try {
            // load the Creditor
            creditorList = CreditorService.getCreditorInstance().getAllCreditor();
            
            if ( creditorList != null )
                LOGGER.log( Level.INFO,  "successfully loaded all Creditors" );
        }
        catch( Throwable exc ) {
            LOGGER.log( Level.WARNING,  "failed to load all Creditors ", exc );
        	return null;
        }

        return creditorList;
                            
    }



//************************************************************************    
// Attributes
//************************************************************************
    protected Creditor creditor = null;
    private static final Logger LOGGER = Logger.getLogger(CreditorQueryRestController.class.getName());
    
}
