/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * FinTech Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.query;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;
/**
 * <h2>WalletQueryRestController as REST API</h2>
 * <p>
 * Implements Spring Controller query CQRS processing for entity Wallet.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>FinTech Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core digital banking, payments, wallets, lending, investments, FX, compliance, and risk management capabilities.</td></tr>
 *          <tr><td>industry</td><td>FinTech</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	WalletService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateWalletCommand</li>
 *      <li></li>UpdateWalletCommand</li>
 *      <liDeleteWalletCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */
@CrossOrigin
@RestController
@RequestMapping("/WalletQuery")
public class WalletQueryRestController extends BaseSpringRestController {
	
    /**
     * Handles loading a Wallet using a UUID
     * @param		UUID walletId
     * @return		Wallet
     */    
    @GetMapping("/load")
    public Wallet load( @RequestParam(required=true) UUID walletId ) {
    	Wallet entity = null;

    	try {  
    		entity = WalletService.getWalletInstance().getWallet( new WalletFetchOneSummary( walletId ) );
        }
        catch( Throwable exc ) {
            LOGGER.log( Level.WARNING, "failed to load Wallet using Id " + walletId );
            return null;
        }

        return entity;
    }

    /**
     * Handles loading all Wallet business objects
     * @return		Set<Wallet>
     */
    @GetMapping("/")
    public List<Wallet> loadAll() {                
    	List<Wallet> walletList = null;
        
    	try {
            // load the Wallet
            walletList = WalletService.getWalletInstance().getAllWallet();
            
            if ( walletList != null )
                LOGGER.log( Level.INFO,  "successfully loaded all Wallets" );
        }
        catch( Throwable exc ) {
            LOGGER.log( Level.WARNING,  "failed to load all Wallets ", exc );
        	return null;
        }

        return walletList;
                            
    }



//************************************************************************    
// Attributes
//************************************************************************
    protected Wallet wallet = null;
    private static final Logger LOGGER = Logger.getLogger(WalletQueryRestController.class.getName());
    
}
